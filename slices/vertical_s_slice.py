...
def create_vertical_s_slice(images, position="center", linear=False, progress_callback=None):
    """
    创建垂直 S 型曲线时间切片（无缝、无黑缝）。

    linear=False（同一位置）：各照片按直线竖向条带拼接（等价于普通垂直切片）。
    linear=True（不同位置）：相邻照片之间以 S 形曲线分隔，形成经典 S 型过渡。

    算法：用 n 条等距 S 形分隔线将整图在水平方向划分为 n 个区域，
    ...
    strip_w = img_w / n
    # S 形摆幅：必须小于半条带宽，确保相邻分隔线不交叉（否则会重叠/错位）
    amp = strip_w * 0.45 if linear else 0.0
